  ...
  const shareEmbed = (type) => {
    if (type == 0) {
      navigator.clipboard.writeText(
        '<iframe src="' + window.location.host + "/embed/" + assistant + '" />',
      );
    } else {
      navigator.clipboard.writeText(
        window.location.host + "/embed/" + assistant,
      );
    }
  };
      ...
      if (data.assistant.tools) {
        data.assistant.tools.forEach((tool) => {
          if (tool.type == "function") {
            ...
            setTypes((prevTypes) => [...prevTypes, tool.type]);
          }
        });
      }
      if (data.assistant.files) {
    ...
    <main className="flex min-h-screen flex-col  bg-myBg ">
      ...
      <div className="max-w-3xl px-2 md:px-8 py-6 flex flex-col gap-5 text-gray-800">
        ...
        <div>
          ...
          <div className="flex flex-col gap-3 text-sm">
            <label className="relative inline-flex items-center cursor-pointer">
              ...
              />
              <div
                className={`w-9 h-5  rounded-full peer     after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-mySecondary  after:rounded-full after:w-4 after:h-4 after:transition-all ${types.includes("code_interpreter") ? "after:translate-x-full rtl:after:-translate-x-full after:border-white bg-myPrimary" : "bg-myBg"}`}
              ></div>
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={() => addType("retrieval")}
              />
              <div
                className={`w-9 h-5  rounded-full peer     after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-mySecondary  after:rounded-full after:w-4 after:h-4 after:transition-all ${types.includes("retrieval") ? "after:translate-x-full rtl:after:-translate-x-full after:border-white bg-myPrimary" : "bg-myBg"}`}
              ></div>
              <span className="ms-3 font-medium ">Retrieval</span>
            </label>
            <div className="flex items-center gap-5 cursor-pointer">
              <div
                className=" rounded-full bg-myBg text-mySecondary text-xl font-bold px-2 w-min"
                onClick={() => {
          ...
          {functions.map((fn, index) => (
            <div key={index} className="relative">
              <textarea
                ...
                value={fn}
                onChange={(e) => addFunction(index, e.target.value)}
              />
              <div
                className="absolute z-10 top-1 right-4 font-bold cursor-pointer"
        ...
        <div className="flex flex-col gap-2">
          <label className=" text-sm font-medium " htmlFor="user_avatar">
            Upload files
          </label>
          <input
            className=" text-sm border border-gray-300 rounded-lg p-2 cursor-pointer bg-gray-50 focus:outline-none"
          ...
          <div className="flex gap-2">
            {files.map((file, index) => (
              <div
                key={index}
                className="text-xs w-min whitespace-nowrap border border-gray-400 py-1 px-2 rounded-xl flex gap-1"
              >
                {file.name}{" "}
                <b className=" cursor-pointer" onClick={() => removeFile(file)}>
                  x
                </b>
              </div>
            ))}
      ...
      {showShare && (
        <div className="h-full grow px-2 md:px-8 py-6 flex flex-col gap-5 text-gray-800">
          ...
          <iframe
            src={"/embed/" + assistant}
            className="h-full grow rounded-xl border"
          />
        </div>
      )}
    </main>
